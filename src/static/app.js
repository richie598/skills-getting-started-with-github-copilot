    ...
    try {
      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        activityCard.innerHTML = `
          ...
          ${details.participants.length > 0 ? `<ul>${details.participants.map(email => `<li>${email} <button class="delete-btn" data-email="${email}" data-activity="${name}">×</button></li>`).join('')}</ul>` : '<p>No participants yet.</p>'}
        `;
